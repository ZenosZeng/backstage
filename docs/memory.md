...
[返回 Backstage 首页](../README.md)

本文说明记忆布局、机器初始化、查询与同步一致性。独立监控和飞书遥控见
...
pixi run sync init --push
pixi run sync sync
```

后续机器使用上面的 `init --pull`。不要直接复制原机器的 `config.json`，也不要为了
绕过已有远端保护而添加 `--force`。同步凭据由本机 `mc` 管理，不填写到 JSON 中。
