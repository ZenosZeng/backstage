...
| **v0.1.1** | 2026-09-09 | Watchdog 从评测日志头识别当前 run 配置、失败告警按运行态分级；飞书进度上报间隔可配；对应回归测试 |
| **v0.1.0** | 2026-09-07 | Backstage 工作区建立：共享记忆与跨机同步、Pi/B1K watchdog、飞书遥控、Pixi 统一环境 |

> **2026-09-14 版本号重排**：此前把改动一律按 minor 递增，纯修复（v0.2.0）和目录迁移（v0.4.0）
> 也占了小版本位，不符合语义化版本。现按 major.minor.patch 重排，仅「飞书遥控对齐终端」
> 算中版本。旧号 → 新号：v0.5.0→**v0.2.2**、v0.4.0→**v0.2.1**、v0.3.0→**v0.2.0**、v0.2.0→**v0.1.1**。
> 2026-09-14 之前的记忆事件与跨机广播里出现的旧号（如「共享区目录重排 v0.4.0」）按此对应，
> 内容本身不受影响。
...
- `resolve-shared` 整包上传前也检查内容回退：本地仍为基线旧版、双方都改过且内容不同，或远端已删除而本地仍保留基线原文，均拒绝上传并列出路径。哈希无法判定第三版是否已正确合并；请保留本地修改，先从冲突快照 `.local/.sync/conflict/remote` 对齐这些路径（含采纳删除）并 resolve，再重新应用、核对和发布本地修改。本地单侧修改、双方内容一致可正常通过。删除护栏仅拦截未改动的基线原文；远端删除、本地已编辑的文件仍由人工决定是否保留。
