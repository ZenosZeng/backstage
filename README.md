...
| 版本 | 日期 | 范围 |
|---|---|---|
| **v0.2.2 · 当前** | 2026-09-14 | 告警重发间隔可配置（默认 600s，降低推送频率）；**生命周期告警不再被限流**——进程启动/结束/异常退出是边沿事件，每次都是新事件，统一限流会让「死亡 → 重启 → 很快再次死亡」只报第一次，现在只有持续型告警（如 stalled）参与限流；补 runner 限流回归 4 例 |
